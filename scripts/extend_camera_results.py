...
def ExtendCameraResult(source_camera_json, target_camera_extend_json, split_json, split_data=None):
    ...
    # here we change some codes because the json name of DCA are slightly different from that of DanceCamera3D
    # for DanceCamera3D, camera_json_name is like 'c{data number}_{split number}_{slice number/test}.json' or 'c{data number}_{slice number/test}.json' for not split data
    # for DCA, camera_json_name is like 'c{data number}_{split number}.json' or 'c{data number}.json' for not split data
    # it's not very smart to change the code here, but it's a very direct way
    camera_json_name = camera_json_name[:-5] + '_0'+camera_json_name[-5:]
